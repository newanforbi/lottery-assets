...
import { TIMELINE_START, TIMELINE_END } from "../data/assets.js";
import { toTime, conflicts, canAdd, sortChain, chainValue } from "../engine/solver.js";
...
export default function Timeline({
  ...
  const openLegs = sorted.filter((l) => l.open);
    ...
    <div>
      ...
      >
        <div style={{ minWidth: compact ? 640 : "auto" }}>
          ...
          {lanes.map((asset) => {
            const assetLegs = legs.filter((l) => l.assetId === asset.id);
            const anySelected = assetLegs.some((l) => selectedIds.has(l.id));
            return (
