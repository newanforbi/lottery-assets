    ...
    <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
      {chain.map((leg, i) => (
        <span key={leg.id} style={{ display: "flex", alignItems: "center", gap: 5 }}>
          {i > 0 && <span style={{ fontFamily: MONO, fontSize: 9, color: "var(--ink-20)" }}>→</span>}
          <span
            style={{
              fontFamily: MONO, fontSize: compact ? 9 : 10, padding: "2px 7px", borderRadius: 4,
              color: ac(leg.color), background: `${ac(leg.color)}14`, border: `1px solid ${ac(leg.color)}33`,
              whiteSpace: "nowrap",
            }}
          >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Panel accent="#00E5FF" title="What the solver found">
        <p style={{ fontFamily: SANS, fontSize: 13.5, color: "var(--ink-60)", lineHeight: 1.7, margin: "0 0 12px" }}>
          ...
          <span style={{ color: ac("#00E5FF"), fontFamily: MONO }}>
            {optimal.chain.map((l) => l.id).join(" → ")}
          </span>{" "}
          at <span style={{ color: ac("#F4B728"), fontFamily: MONO }}>{formatMultiple(optimal.value)}</span>, ahead of
          ...
          <span style={{ fontFamily: MONO, color: ac("#23F0C6") }}>5.71×</span>, beating SuperVerse's{" "}
          <span style={{ fontFamily: MONO, color: ac("#FF4FD8") }}>4.91×</span> and Strategy's{" "}
          <span style={{ fontFamily: MONO, color: ac("#FF7A45") }}>3.53×</span> in the same gap between
        ...
        <p style={{ fontFamily: SANS, fontSize: 13.5, color: "var(--ink-60)", lineHeight: 1.7, margin: 0 }}>
          ...
          <span style={{ fontFamily: MONO, color: ac(single.color) }}>
            {single.ticker} at {formatMultiple(single.multiple)}
          </span>. Chaining is worth roughly{" "}
          <span style={{ fontFamily: MONO, color: "var(--ink)" }}>{Math.round(optimal.value / single.multiple)}×</span>{" "}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isOptimal ? `${ac("#00E5FF")}12` : "var(--fill-025)",
                  border: `1px solid ${isOptimal ? `${ac("#00E5FF")}59` : "var(--line-055)"}`,
              ...
              >
                <span
                  style={{
                    ...
                    color: isOptimal ? ac("#00E5FF") : "var(--ink-30)", fontWeight: 600,
                  }}
                >
                  {i + 1}
                </span>
                ...
                <span
                  style={{
                    ...
                    color: isOptimal ? ac("#F4B728") : "var(--ink-80)",
                ...
                <span
                  style={{
                    fontFamily: MONO, fontSize: 10, color: "var(--ink-35)",
      ...
      <Panel title="Checking the original nine paths">
        <p style={{ fontFamily: SANS, fontSize: 12.5, color: "var(--ink-45)", lineHeight: 1.65, margin: "0 0 14px" }}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: valid ? "var(--fill-025)" : `${ac("#FF3C3C")}0D`,
                  border: `1px solid ${valid ? "var(--line-055)" : `${ac("#FF3C3C")}3D`}`,
                }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 9, flexWrap: "wrap" }}>
                    <span style={{ fontFamily: MONO, fontSize: 9, color: "var(--ink-25)" }}>#{p.rank}</span>
                    <span style={{ fontFamily: SANS, fontSize: 12.5, color: "var(--ink-75)" }}>{p.label}</span>
                    {valid ? <Chip color="#4BE04B">EXECUTABLE</Chip> : <Chip color="#FF3C3C">OVERLAPS</Chip>}
                  </div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                    <span style={{ fontFamily: MONO, fontSize: 13, fontWeight: 600, color: valid ? "var(--ink)" : "var(--ink-40)" }}>
                      {formatMultiple(value)}
                    </span>
                    <span style={{ fontFamily: MONO, fontSize: 9, color: matches ? (isLight ? ac("#4BE04B") : `${ac("#4BE04B")}99`) : ac("#FF3C3C") }}>
                      {matches ? "matches published" : `published ${formatMultiple(p.claimed)}`}
                    </span>
                  </div>
                </div>
                {clash && (
                  <div style={{ marginTop: 9, fontFamily: MONO, fontSize: 10.5, color: `${ac("#FF7878")}D9`, lineHeight: 1.6 }}>
